...
- **FR5**: Prompt mồi Whisper SHALL trung tính (chỉ làm mẫu ngắt câu, không chứa nội dung
  chủ đề cụ thể).
  - ~~WHERE người dùng có Từ điển tên riêng, hệ thống SHALL đưa các tên gốc vào prompt~~
    **[ĐÃ BỎ ở spec 07]**: vế này từng được thêm để Whisper nghe/viết đúng tên nhân vật,
    nhưng gây rủi ro lớn hơn lợi ích. Bằng chứng thực tế (2026-07): 2 máy cùng file video,
    cùng cài đặt, chỉ khác nội dung Từ điển (chứa tên từ dự án KHÁC, không liên quan video
    đang chạy) → chênh lệch 138 vs 112 dòng phụ đề (thiếu 19%, đo lặp lại nhiều lần, ổn
    định — không phải nhiễu ngẫu nhiên). Nguyên nhân: prompt của Whisper là "ngữ cảnh mồi"
    (theo tài liệu OpenAI), nội dung mồi không liên quan làm mô hình lệch hướng giải mã,
    bỏ nguyên cụm câu — cùng cơ chế đã thấy ở phần chia khúc (spec 06). Bài học: mọi thay
    đổi đưa nội dung do người dùng nhập vào prompt AI cần test cả kịch bản nội dung KHÔNG
    liên quan/lỗi thời, không chỉ test kịch bản đúng như kỳ vọng.
